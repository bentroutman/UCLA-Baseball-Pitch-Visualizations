catcherView <- function(csv, pitcher_name) {
  # csv contains Trackman data
  
  library(plotly)
  library(shiny)
  library(dplyr)
  current_view <- reactiveVal("catcher")
  ...
  ui <- fluidPage(
    actionButton("start", "Animate Pitch"),
        ...
        for (j in seq_len(nrow(pitch))) {
          ...
          plotlyProxy("pitchPlot", session) %>%
            plotlyProxyInvoke("restyle", list(
              ...
              marker = list(list(size = 8, color = color))
            ), list(2))
...
catcherView("C:/Users/johnt/Downloads/Pitch and Hit Tracking (1)/Pitch and Hit Tracking/20240518-UCLA-1.csv", "Rodriguez, Luke")
